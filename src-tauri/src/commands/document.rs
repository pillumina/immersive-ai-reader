...
use crate::models::document::{Document, DocumentSummary, CreateDocumentRequest};
use anyhow::Result;
...
pub async fn create_document(
    ...
    // Use atomic upsert_by_file_path: if file_path already exists, preserves id/conversations
    // and updates metadata; otherwise inserts a new document. No TOCTOU race possible.
    let doc = Document::from_request(request);
    repo.upsert_by_file_path(&doc).await.map_err(|e| e.to_string())
}
...
pub async fn get_all_documents(
    ...
    repo.get_all_summaries().await.map_err(|e| e.to_string())
}
...
pub async fn read_pdf_file(path: String) -> Result<Vec<u8>, String> {
    fs::read(&path).await.map_err(|e| format!("Failed to read file '{}': {}", path, e))
}
...
pub async fn get_documents_by_library(
    ...
    repo.get_summaries_by_library(&library_id)
        .await
        .map_err(|e| e.to_string())
}
