...
impl DocumentRepository {
    ...
    pub async fn save(&self, doc: &Document) -> Result<()> {
        sqlx::query(
            r#"
            INSERT INTO documents (id, file_name, file_path, file_size, page_count, text_content, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    pub async fn get_by_id(&self, id: &str) -> Result<Option<Document>> {
        let doc = sqlx::query_as(
            r#"SELECT id, file_name, file_path, file_size, page_count, text_content, created_at, updated_at FROM documents WHERE id = ?"#,
        )
        .bind(id)
    ...
    pub async fn get_all(&self) -> Result<Vec<Document>> {
        let docs = sqlx::query_as(
            r#"SELECT id, file_name, file_path, file_size, page_count, text_content, created_at, updated_at FROM documents ORDER BY created_at DESC"#
        )
        .fetch_all(&self.pool)
        .await?;
