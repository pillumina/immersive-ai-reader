...
- `docs/PRETEXT_INTEGRATION_DESIGN.md`（Pretext 集成详设 v1.0）

## Current Goal

Focus Mode Phase 1-4 全部完成 🎉。性能优化系列完成 ✅：虚拟滚动页面回收、网络重试逻辑（指数退避）、内存泄漏修复、Dark Mode、Debounced 自动保存、AI 使用量 Dashboard、日志改造（tracing 文件日志 + 查看器）。下一步：**Pretext 集成优化**（详见 `docs/PRETEXT_INTEGRATION_DESIGN.md`）。
...
- [x] AI 会话上下文恢复（共享 conversation，进入/退出 Focus Mode 自动切换，Resume Prompt 显示对话预览）
- [x] 首次使用 Tooltip（`localStorage` 标记，Focus Mode 入口显示快捷键提示）
- [x] 动画性能优化（MiniAIWindow / CaptureDrawer / FocusStatusBar / scroll-container `will-change` hints）
- [x] Focus Session 历史记录（CaptureDrawer 会话 Tab：显示历史会话卡片，包含日期/时长/页码/进度/捕获数量，支持继续阅读和删除）

---

## Pretext Integration (2026-03-30)

详设文档：`docs/PRETEXT_INTEGRATION_DESIGN.md`

### Phase 1：精确高亮矩形

- [ ] 安装 `@chenglou/pretext` + 类型定义
- [ ] `pretext-text-layer.ts`：从 pdfjs TextContent 构建 Pretext 排版数据
- [ ] `pretext-hit-test.ts`：坐标→文本映射 + 选区→高亮矩形
- [ ] `pretext-line-cache.ts`：LRU 缓存（最大 50 页）
- [ ] `renderer.ts`：渲染后缓存行数据
- [ ] `useCanvasRendering.ts`：highlightSelection 优先使用行数据

### Phase 2：Canvas 文本层

- [ ] `pretext-text-renderer.ts`：Canvas 透明文本渲染
- [ ] 隐藏文本 div（每行 1 span，支持原生选择 + 搜索）
- [ ] `renderer.ts`：替代 pdfjs TextLayer

### Phase 3：虚拟化文本层

- [ ] `usePretextTextLayer.ts`：IntersectionObserver 驱动
- [ ] 离屏页面回收
...
- 2026-03-26: Focus Mode Phase 4 fully complete: Focus Session 历史记录面板（CaptureDrawer 新增"会话" Tab，后端 get_all_focus_sessions + delete_focus_session 命令，前端 focusCommands.getAllSessions + deleteSession，会话卡片显示日期/时长/页码/进度/捕获数量，支持继续阅读跳转和删除）。
